...
price, and
*      multiplies the results. There is a known (minor) issue with the getRoundData() function,
where the historical
*      value for a previous round (price) can be queried from the feed, but the current st/wstEth
rate is used
*      (instead of the historical pair); we do not see that as a problem as this contract's return
values are
