...
contract VesselManager is IVesselManager, UUPSUpgradeable, ReentrancyGuardUpgradeable, TrenBase {
    // Constants
    // ------------------------------------------------------------------------------------------------------
    ...
    * L_Colls and L_Debts track the sums of accumulated liquidation rewards per unit staked. During
     ...
     *
    * Where L_Colls(0) and L_Debts(0) are snapshots of L_Colls and L_Debts for the active Vessel
    ...
    // Array of all active vessel addresses - used to to compute an approximate hint off-chain, for
        ...
        returns (bool)
    {
        ...
        return nextVessel == address(0)
            || getCurrentICR(_asset, nextVessel, _price) < IAdminContract(adminContract).getMcr(_asset);
    }

    // Return the nominal collateral ratio (ICR) of a given Vessel, without the price. Takes a
    // vessel's pending coll and debt rewards from redistributions into account.
    ...
    // Return the current collateral ratio (ICR) of a given Vessel. Takes a vessel's pending coll
    ...
    // Move a Vessel's pending debt and collateral rewards from distributions, from the Default Pool
    // to the Active Pool
        ...
        returns (uint256 stake)
    {
        ...
        if (assetColl == 0) {
            ...
            * - When we close or liquidate a vessel, we redistribute the pending rewards, so if all
            vessels were closed/liquidated,
    ...
    // --- Vessel property setters, called by Tren's
