...
export default function shouldBehaveLikeCanAuthorizeUpgrade(): void {
  context("when caller is owner", function () {
    it("should flashLoan updated", async function () {
      const { wETH } = this.collaterals.active;

      await this.contracts.flashLoan.connect(this.signers.deployer).authorizeUpgrade(wETH.address);
    });
  });
  ...
  context("when caller is not an owner", function () {
    it("reverts custom error", async function () {
      ...
      await expect(
        this.contracts.flashLoan.connect(impostor).authorizeUpgrade(wETH.address)
