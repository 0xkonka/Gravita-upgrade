...
export default function shouldHaveGetAssetBalance(): void {
  context("for active collateral", function () {
    it("should return correct borrowing fee", async function () {
      const { wETH } = this.collaterals.active;

      const assetBalance = await this.contracts.activePool.getAssetBalance(wETH.address);
  ...
  context("for inactive collateral", function () {
    it("should return BORROWING_FEE_DEFAULT", async function () {
      ...
      const assetBalance = await this.contracts.activePool.getAssetBalance(dai.address);
  ...
  context("for non-existent collateral", function () {
    it("should return ZERO", async function () {
      ...
      const assetBalance = await this.contracts.activePool.getAssetBalance(nonExistentCollateral);
