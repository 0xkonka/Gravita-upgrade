...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  await preDeploy(deployer, "SortedTrenBoxes");
  const deterministicDeploy = await deterministic("SortedTrenBoxes", {
    from: deployer,
    proxy: {
        execute: {
          init: {
  ...
  if (isLocalhostNetwork(chainId) === false) {
    const contractPath = `contracts/SortedTrenBoxes.sol:SortedTrenBoxes`;
...
export default func;
func.id = "deploy_SortedTrenBoxes";
func.tags = ["SortedTrenBoxes"];
