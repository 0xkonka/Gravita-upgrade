...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  await preDeploy(deployer, "AdminContract");
  const deterministicDeploy = await deterministic("AdminContract", {
    from: deployer,
    proxy: {
      execute: {
        init: {
  ...
  if (isLocalhostNetwork(chainId) === false) {
    const contractPath = `contracts/AdminContract.sol:AdminContract`;
...
export default func;
func.id = "deploy_AdminContract";
func.tags = ["AdminContract"];
