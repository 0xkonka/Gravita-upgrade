...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  await preDeploy(deployer, "CollSurplusPool");
  const deterministicDeploy = await deterministic("CollSurplusPool", {
    from: deployer,
    proxy: {
        execute: {
          init: {
  ...
  if (isLocalhostNetwork(chainId) === false) {
    const contractPath = `contracts/CollSurplusPool.sol:CollSurplusPool`;
...
export default func;
func.id = "deploy_CollSurplusPool";
